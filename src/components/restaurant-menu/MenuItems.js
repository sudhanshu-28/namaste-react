...
const ItemLeftContainer = ({ item }) => {
  const productInfo = item?.card?.info;

  return (
    <div className="flex-[7] space-y-1">
      <div className="font-bold text-gray-600 text-lg">{productInfo?.name}</div>
      <div className="text-gray-900 font-bold">{`₨.${
        (productInfo?.price || productInfo?.defaultPrice) / 100
      }`}</div>
      {productInfo?.ratings?.aggregatedRating?.rating && (
        <div className="text-sm font-bold space-x-1">
          <span className="text-green-800">{`⁕ ${productInfo?.ratings?.aggregatedRating?.rating}`}</span>
          <span className="text-gray-500">
            ({productInfo?.ratings?.aggregatedRating?.ratingCountV2})
          </span>
        </div>
      )}

      <div className="line-clamp-2 text-gray-500 pr-6">
...
const ItemRightContainer = ({ item }) => {
  ...
  const productInfo = item?.card?.info;
  const cloudinaryImageId = productInfo?.imageId;
  ...
  const isItemInCart = items.find(
    (el) => el?.card?.info?.id === productInfo?.id
  );
    ...
    <div className="flex-[2] w-32 h-32 rounded-2xl p-2 flex items-center justify-center relative">
      ...
      {isItemInCart ? (
        <div className={addBtnStyle + controlBtnStyle}>
          ...
          <button disabled>{isItemInCart?.card?.quantity || 0}</button>
